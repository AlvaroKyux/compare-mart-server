/**
 * Inserta sucursales reales (verificadas vÃ­a Google Places) para el feature
 * de sensor de ubicaciÃ³n. Uso: npm run seed:sucursales
 * (requiere haber corrido "npm run migrate" antes).
 *
 * Alcance geogrÃ¡fico: Pachuca de Soto, Hidalgo â€” zona de prueba del equipo.
 * Solo 6 de las 10 cadenas del catÃ¡logo tienen presencia aquÃ­; HEB, Costco,
 * La Comer y Fresko no tienen sucursal en la ciudad y quedan fuera de esta
 * tabla a propÃ³sito (ver nota en schema.sql).
 *
 * Este script es idempotente: si se corre mÃ¡s de una vez, evita duplicar
...
const SUCURSALES = [
  ...
  },
  {
    ...
    direccion: 'Blvd. Felipe Ãngeles km 87.8, Venta Prieta, 42084 Pachuca de Soto, Hgo.',
    latitud: 20.0902973,
    longitud: -98.7729083,
  },
  {
    cadena: 'samsClub',
    nombre_sucursal: "Sam's Club RÃ­o de las Avenidas",
    direccion: 'Viad. RÃ­o de las Avenidas 701, Terrazas, 42098 Pachuca de Soto, Hgo.',
    latitud: 20.1134972,
    longitud: -98.7411499,
  },
  {
    cadena: 'bodegaAurrera',
    nombre_sucursal: 'Bodega Aurrera Everardo MÃ¡rquez',
    direccion: 'Blvd Everardo MÃ¡rquez 101, Periodistas, 42060 Pachuca de Soto, Hgo.',
    latitud: 20.1137446,
    longitud: -98.7455385,
  },
  {
  ...
  {
    cadena: 'bodegaAurrera',
    nombre_sucursal: 'Bodega Aurrera RamÃ³n G. Bonfil',
    direccion: 'Blvd. RamÃ³n G. Bonfil 1509-I, El Palmar, 42088 Pachuca de Soto, Hgo.',
    latitud: 20.1124298,
    longitud: -98.7739284,
  },
  {
    cadena: 'soriana',
    nombre_sucursal: 'Soriana HÃ­per Pachuca',
  ...
  {
    cadena: 'chedraui',
    nombre_sucursal: 'SÃºper Chedraui Explanada',
    direccion: 'Carr. MÃ©xico - Pachuca Km 82, San Antonio el Desmonte, 42119 Pachuca de Soto, Hgo.',
    latitud: 20.0369303,
    longitud: -98.7968417,
  },
  {
...
async function seedSucursales() {
  ...
  try {
    for (const s of SUCURSALES) {
      // No hay una restricciÃ³n UNIQUE natural sobre sucursales fÃ­sicas (dos
      // sucursales de la misma cadena pueden tener el mismo nombre comercial
      // en ciudades distintas), asÃ­ que la idempotencia se resuelve aquÃ­ en
      // aplicaciÃ³n: solo insertar si no existe ya la misma cadena+nombre.
      const { rows } = await pool.query(
        'SELECT id FROM sucursales WHERE cadena = $1 AND nombre_sucursal = $2',
        [s.cadena, s.nombre_sucursal]
      );
      if (rows.length > 0) {
        console.log(`  â€“ Ya existe: [${s.cadena}] ${s.nombre_sucursal}`);
        continue;
      }
      await pool.query(
      ...
      console.log(`  âœ” Insertada: [${s.cadena}] ${s.nombre_sucursal}`);
    }
    console.log('âœ” Seed de sucursales completado.');
  } catch (err) {
    console.error('âœ˜ Error al insertar sucursales:', err.message);
    process.exitCode = 1;
  } finally {
    await pool.end();
  }
